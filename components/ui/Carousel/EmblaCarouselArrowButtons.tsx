...
export const usePrevNextButtons = (
    ...
    const onPrevButtonClick = useCallback(() => {
        if (!emblaApi) return
        emblaApi.scrollPrev() // Fixed
    }, [emblaApi])

    const onNextButtonClick = useCallback(() => {
        if (!emblaApi) return
        emblaApi.scrollNext() // Fixed
    }, [emblaApi])

    const onSelect = useCallback((emblaApi: EmblaCarouselType) => {
        setPrevBtnDisabled(!emblaApi.canScrollPrev()) // Fixed
        setNextBtnDisabled(!emblaApi.canScrollNext()) // Fixed
    }, [])
